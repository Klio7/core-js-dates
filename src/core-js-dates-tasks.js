...
function isDateInPeriod(date, period) {
  ...
  const checkedDate = Date.parse(date);
  return checkedDate >= start && checkedDate <= end;
}
...
function formatDate(date) {
  ...
  const [year, month, dateOfMonth, hours, minutes, seconds] = [
    exactDate.getFullYear(),
    exactDate.getMonth(),
    exactDate.getUTCDate(),
    exactDate.getUTCHours(),
    exactDate.getUTCMinutes().toString().padStart(2, '0'),
    exactDate.getUTCSeconds().toString().padStart(2, '0'),
  ];
  if (hours > 12) {
    let newHours = hours;
    newHours = hours % 12;
    return `${month + 1}/${dateOfMonth}/${year}, ${newHours}:${minutes}:${seconds} PM`;
  }
  if (hours === 12) {
    return `${month + 1}/${dateOfMonth}/${year}, ${hours}:${minutes}:${seconds} PM`;
  }

  return `${month + 1}/${dateOfMonth}/${year}, ${hours}:${minutes}:${seconds} AM`;
}
...
function getWeekNumberByDate(date) {
  ...
  const numberOfDays =
    Math.floor((date - firstJan) / (24 * 60 * 60 * 1000)) -
    daysOfWeek[firstJanDay];
  return Math.ceil((numberOfDays + 1) / 7) + (firstJanDay === 1 ? 0 : 1);
}
